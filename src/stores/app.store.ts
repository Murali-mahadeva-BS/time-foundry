...
import type { Project, List, Task, Status, PomodoroSession } from '@/types'
...
export const useAppStore = create<AppStore>((set, get) => ({
  ...
  syncPendingSessions: async () => {
    ...
    if (enriched.length > 0) {
      await db.sessions.bulkPut(enriched)
    }
  ...
  createProject: async (name, description, importFromProjectId, icon) => {
    ...
    if (importFromProjectId) {
      ...
      if (source) {
        statuses = source.statuses.map((s) => ({ ...s, id: generateId() }))
      }
    }
